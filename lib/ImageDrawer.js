...
const assetsDir = path.join(__dirname, "../resources/img/")
...
async function loadAssets() {
  try {
    ...
    console.info("[飞行棋] 图像资源加载成功！")
  } catch (error) {
    console.error("[飞行棋] 加载图像资源时出错:", error)
  }
}
...
async function getAvatar(url, size = 40) {
  try {
    ...
    console.error(`获取头像失败: ${url}`, error)
    return null
  }
}
...
async function drawGameBoard(game, diceResult = 0) {
  if (!ASSETS_CACHE.background) {
    console.error("[飞行棋] 背景图片未加载，无法绘制。")
    return null
  }
