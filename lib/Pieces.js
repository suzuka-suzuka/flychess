...
const Rule = {
  win: (piece, color) => console.info(`${color}方棋子 ${piece.id} 获胜了！`),
  tipsJump: () => console.info("跳到了下一个同色格！"),
  tipsFly: () => console.info("超级飞行！"),
  attack: () => console.info("攻击了对方棋子！"),
}

// 事件收集器
class MoveEventCollector {
  constructor() {
    this.events = []
  }
...
class Piece {
  ...
  jumpStep(r, allPieces, ruleCallbacks = Rule, eventCollector = null) {
    ...
    for (let i = 1; i <= r; i++) {
      ...
      if (opponentsOnPathPos.length > 0) {
        ...
        console.info(`路径上在 ${stepsToStack} 步处遇到敌方迭子!`)
        const excessSteps = r - stepsToStack

        if (excessSteps > 0) {
          console.info(`点数大于与迭子的距离，从迭子处后退 ${excessSteps} 步。`)
  ...
  reset() {
    ...
    this.isStacked = false
    console.info(`棋子 ${this.id} 被送回了基地。`)
  }
  _testState(allPieces, R, r) {
    ...
    if (this.position.color === this.color && this.position.s) {
      ...
      this.position = this._getNextColor()
      R.tipsJump()
  ...
  _tryAttack(allPieces, R, r) {
    ...
    if (opponentsOnSpot.length >= 2) {
      console.info(`降落在敌方迭子上！攻击方与迭子均返回停机坪。`)
  ...
  _updateStackStatusAtPosition(positionId, allPieces) {
    ...
    colorsOnSpot.forEach(color => {
      ...
      if (isNowStacked && !sameColorPieces[0].isStacked) {
        console.info(`颜色 ${color} 在位置 ${positionId} 形成了迭子。`)
      }
  ...
  _getNextColor() {
    ...
    while (true) {
      ...
      if (nextPos && nextPos.color === this.color) {
        return nextPos
      }
    }
  }
